  ...
  return (
    <div className="print-report-container print-container p-8 font-sans text-stone-900 bg-white leading-relaxed text-xs">
      <div className="flex justify-between items-start border-b-2 border-stone-800 pb-5 mb-5">
        <div>
          <h1 className="text-base font-bold uppercase tracking-tight text-stone-900">{storeSettings?.storeName || 'SmarTech Repair'}</h1>
          <p className="text-[10px] text-stone-500">{storeSettings?.address || 'Atelier de réparation'}</p>
...
export default function Dashboard({ userProfile }: DashboardProps) {
  ...
  // Réparations (résumé atelier)
  const [repairs, setRepairs] = useState<any[]>([]);
  useEffect(() => {
    const unsub = onSnapshot(query(collection(db, 'repairs'), where('ownerId', '==', ownerId)), (snap) => {
      setRepairs(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    }, (err) => console.warn('repairs snapshot error', err));
    return () => unsub();
  }, [ownerId]);
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* Résumé Atelier de réparation */}
      {(() => {
        const active = repairs.filter(r => ['recu','diagnostic','en_attente_piece','en_cours','termine'].includes(r.status)).length;
        const ready = repairs.filter(r => r.status === 'termine').length;
        const waiting = repairs.filter(r => r.status === 'en_attente_piece').length;
        const unpaid = repairs.reduce((s, r) => s + (r.debt || 0), 0);
        const cur = storeSettings?.currency || 'DT';
        const cards = [
          { label: 'Réparations en cours', value: active, color: 'from-indigo-500 to-indigo-600' },
