...
export const PrintableTicket = forwardRef<HTMLDivElement, Props>(({ invoice, ownerId }, ref) => {
  ...
  return (
    <div ref={ref} className="bg-white text-black font-mono" style={{ width: '80mm', margin: '0 auto', padding: '8px' }}>
      {/* Titre SmarTech */}
      <div style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '16px', letterSpacing: '0.5px', marginBottom: '4px', textTransform: 'uppercase' }}>
        SmarTech
      </div>
      <div style={{ borderTop: '1px dashed black', margin: '3px 0' }}></div>
      
      {/* Date et heure */}
      <div style={{ textAlign: 'center', fontSize: '9px', fontWeight: '600', marginBottom: '3px' }}>
        {dateStr}
      </div>
      ...
      <div style={{ marginTop: '4px', marginBottom: '4px' }}>
        {invoice.items.map((item, index) => (
          <div key={index} style={{ borderBottom: '1px dashed #ccc', paddingBottom: '3px', marginBottom: '3px' }}>
            {/* Code à barre si disponible */}
            {item.barcode && (
              <div style={{ textAlign: 'center', margin: '2px 0', fontSize: '8px', fontWeight: 'bold', wordBreak: 'break-all', letterSpacing: '0.5px' }}>
                {item.barcode}
              </div>
            )}
            
            {/* Nom de l'article */}
            <div style={{ fontWeight: 'bold', fontSize: '10px', textAlign: 'center', marginBottom: '2px', wordBreak: 'break-word', lineHeight: '1.2' }}>
              {item.name}
            </div>
            
            {/* Référence si disponible */}
            {item.reference && (
              <div style={{ textAlign: 'center', fontSize: '8px', color: '#444', marginBottom: '2px', fontWeight: '600' }}>
                Réf: {item.reference}
              </div>
            )}
      ...
      {/* Infos facture */}
      <div style={{ textAlign: 'center', fontSize: '8px', marginBottom: '3px' }}>
        <div style={{ fontWeight: '600', marginBottom: '1px' }}>Ticket: {invoice.number}</div>
        <div style={{ fontWeight: '600', fontSize: '8px', wordBreak: 'break-word' }}>{invoice.clientName}</div>
      </div>
      ...
      {invoice.debt > 0 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '9px', color: '#d32f2f', fontWeight: 'bold' }}>
          <span>Reste</span>
      ...
      {/* Footer */}
      <div style={{ textAlign: 'center', marginTop: '3px', fontSize: '8px' }}>
        Merci !
      </div>
    </div>
  );
